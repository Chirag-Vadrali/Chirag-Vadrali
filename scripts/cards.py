...
def main(argv=None):
    ...
    for entry in wanted:
        lookup_name = entry.get("github_repo") or entry["repo"]
        src = by_name.get(lookup_name.lower())
        if not src:
            print(f"  !! {lookup_name} (for {entry['repo']}) not found on the account, skipped")
            continue
        card = {
            "name": entry["repo"],
        ...
        print(f"wrote card-{entry['repo']}-*.svg  "
